...
public class AlunoInfrastructureTests : TestBase
{
    ...
    internal static async Task<Aluno> CriarEInserirAlunoAsync(AlunoRepository alunoRepo, LogradouroRepository logradouroRepo)
    {
        ...
        var alunoResult = Aluno.Criar(
            id: 0,
            nome: "Aluno Alvaro " + Guid.NewGuid().ToString("N")[..5],
            cpf: GerarCpf(),
    ...
    public async Task Aluno_Atualizar_Sucesso()
    {
        ...
        var aluno = await _alunoRepo.Adicionar(Aluno.Criar(
            0, "Aluno Alvaro Original", GerarCpf(), new DateOnly(2000, 10, 20),
            GerarTelefone(), GerarEmail(), logradouro, "100", "Feltrin Original",
            "SenhaValida123SQLite", foto).Value!);

        var novoNome = "Aluno Alvaro Novo " + Guid.NewGuid().ToString("N")[..5];
    ...
    public async Task Aluno_ObterPorNome_FiltragemCorreta()
    {
        ...
        var resultados = await _alunoRepo.ObterPorNome("Alvaro");
        Assert.NotNull(resultados);
        Assert.Contains(resultados, c => c.Id == aluno.Id);
    }
    ...
    {
        var aluno = await CriarEInserirAlunoAsync(_alunoRepo, _logradouroRepo);
        var novaSenha = Senha.Criar("NovaSenhaValida123").Value!;
        ...
        Assert.Equal("NovaSenhaValida123", atualizado.Senha.Valor);
